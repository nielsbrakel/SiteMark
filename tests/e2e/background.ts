 ...
 * a message from an extension page. Returns the new worker; every top-level listener and the
 * start-up sync run again (plan §3.1).
 */
export async function restartServiceWorker(
  ...
  const old = context.serviceWorkers().find((worker) => worker.url().includes(extensionId));
  const closed = old ? new Promise((resolve) => old.once('close', resolve)) : Promise.resolve();
  await cdp.send('ServiceWorker.enable');
  await cdp.send('ServiceWorker.stopAllWorkers');
  await closed;
  const started = context.waitForEvent('serviceworker', { timeout: 10_000 });
