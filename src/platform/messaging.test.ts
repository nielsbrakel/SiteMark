...
const responses = {
  ...
  getTabStatus: status,
  markThisSite: ok({ revision: 2, notices: [] }),
  importPreview: ok({ updated: 0, added: 1, originsToRequest: [], regexPatterns: [] }),
...
function handlers() {
  return {
    ...
    startPicker: vi.fn(async () => responses.startPicker),
    toggleHidden: vi.fn(),
    getTabStatus: vi.fn(async () => responses.getTabStatus),
    markThisSite: vi.fn(async () => responses.markThisSite),
    importPreview: vi.fn(async () => responses.importPreview),
    importApply: vi.fn(async () => responses.importApply),
