...
//   pnpm progress --sync       rewrite the Status column of docs/tasks.md from git
...
function mentionedRequirements(): Set<string> {
  const files = globSync(['src/**/*.test.{ts,tsx}', 'tests/**/*.{test,spec}.{ts,tsx}']);
  return new Set(files.flatMap((f) => readFileSync(f, 'utf8').match(/REQ-[A-Z0-9]+-\d{3}/g) ?? []));
}

/** A path or glob from the Tests column; bare file names may live anywhere under src/ or tests/. */
function fileExists(entry: string): boolean {
  const patterns = entry.includes('/') ? [entry] : [`src/**/${entry}`, `tests/**/${entry}`];
...
function syncStatusColumn(isDone: (task: string) => boolean): void {
  const text = readFileSync('docs/tasks.md', 'utf8').replace(
    /^(\| (T-\d{3}) \|.*\| )(✅|☐)(\s*\|)$/gm,
    (_row, head: string, id: string, _status: string, tail: string) =>
      `${head}${isDone(id) ? '✅' : '☐'}${tail.length > 1 ? tail : ' |'}`,
  );
  writeFileSync('docs/tasks.md', text);
  console.log('docs/tasks.md: Status column synced from git. Run `pnpm format` to realign.');
}
...
function main(): void {
  ...
  const spec = parseRequirements(readFileSync('docs/spec.md', 'utf8'));
  const plan = parseTasks(readFileSync('docs/tasks.md', 'utf8'));
  ...
  printMilestones(plan.milestones, isDone);
  ...
  printRequirements(spec.requirements, reports);
