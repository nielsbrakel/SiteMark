...
| REQ-PRIV-007 | M   | A plain-language `PRIVACY.md` states: no data is collected; the favicon image request; pages can detect injected marks; the title prefix ends up in browser history; exports contain internal hostnames.                                                                                                                                                                                    |
...
| REQ-NFR-004 | M   | **Quality gates** that must pass in CI before merge: typecheck, lint (type-aware, with layer zones), format, stylelint, unit/dom/browser tests with coverage (core ≥ 90 % lines and 85 % branches, platform ≥ 85 %, overall ≥ 80 %), e2e, verify-tdd and commitlint.                 |
