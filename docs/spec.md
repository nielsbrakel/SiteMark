...
| REQ-SEC-003 | M   | **Message validation:** only the background handles requests on `runtime.onMessage`. Content scripts listen there only for the background's pushes (`applyPlan`, `setHidden`, `getStatus`, accepted only from SiteMark outside a tab; `platform/listen-in-tab.ts`). The background checks `sender.id` and the sender type (extension page or top-frame content script), and validates payloads with zod. Responses from content scripts are validated too. No `onMessageExternal`, an empty `externally_connectable`, and no `window.postMessage`. |
