...
| ------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`      | PR, push `main`    | **check** in the Playwright container (typecheck, Biome, stylelint, format, knip, core + dom + browser + scripts with coverage, `progress --strict`) · **build** matrix chrome/firefox/safari (manifest tests, output scan, size budget, `web-ext lint`) · **e2e** (Playwright container) · **traceability** (`progress --coverage` over all reports) · **verify-tdd** (PRs) · **commitlint** (PRs) · **workflow-lint** (actionlint, zizmor) · **dependency-review** (PRs) · **ci-ok** (aggregate; the only required check) |
...
`tests/unit/ci-policy.test.ts` enforces this, and `docs/repo-setup.md` §3a documents the policy.
